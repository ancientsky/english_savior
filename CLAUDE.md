...
- **Alchemy recipes** (`ALCHEMY_RECIPES`): `{ ch, a, b, w, zh, e }` where a/b are `ALCHEMY_PARTS` ids. **Hard rule: `parts[a].text + parts[b].text` must equal `w` exactly** — no doubling, no y→i, so what's on screen IS the rule. `node validate_alchemy.js` (scratchpad) enforces it.
...
All CSS/JS references in index.html carry a `?v=N` query string. GitHub Pages caches assets for 10 minutes, so a freshly deployed index.html can otherwise pair with stale cached JS/CSS. **Bump the version on every release that changes JS or CSS** (single `sed -i 's/?v=51/?v=52/g' index.html`-style edit).
