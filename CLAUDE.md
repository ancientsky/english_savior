...
js/
  ...
  fishing.js            — Cozy Fishing Pond: 2.5D parallax scenes, in-scene cast/reel overlays (hand-held rod + SVG line, bite camera-zoom + vignette, tension-fight with thrashing fish, catch-moment fish leap; scales up in game-max), TTS listening quiz (rare/legendary = spelling), 110 fish across 14 unlockable ponds (10 rarity-4 legendaries paying 30XP+5💎; first-catch calls recordFishDistinct/recordFishLegendary), per-pond aquarium album (FISH_SPECIES in data/fishing.js)
  ...
  data/
    ...
    fishing.js          — Fish species (FISH_SPECIES ×110 across 14 ponds; ponds 5-14 each have a legendary: true rarity-4 fish, invented compounds carry say fields for TTS)
    game.js             — Achievements (68, each with pts; `hidden` ones show ??? until unlocked), daily quests (16), inventory items (8, usable as charms), shop items (consumables/skins/titles/themes), ACH_POINT_REWARDS, LEVEL_MILESTONES, CHARM_PERKS, SELL_PRICES
reference/
...
- **Achievement points**: derived from unlocked achievements (10/20/40 pts tiers, 1,350 total); ACH_POINT_REWARDS thresholds grant exclusive titles/skins/themes (state.pointRewardsClaimed)
