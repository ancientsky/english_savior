...
- **Wizard words** (`WIZARD_WORDS`): `w` (lowercase a-z only — it is compared against the child's spelling), `zh`, `e` (emoji), `tags` (from `WIZARD_TAGS`), `cat` (from `WIZARD_CATS` — the spellbook browses by category, never by ability, because filtering by "floats" is the answer to every river), `lv` (spellbook page 1-5, unlocked per `WIZARD_UNLOCK`). **Obstacles** (`WIZARD_OBSTACLES`): `solve` lists every tag that beats it; `tip` describes the SITUATION and is always on screen; `hint` lists the ways through and only appears behind 「💡 想不出來」, which costs stars. Reaction flags (`flammable`/`freezable`/`douseable`/`lightable`, keyed by `WIZARD_REACTS`) let one summon clear other blockers for free — every reaction tag must already be in that obstacle's own `solve` list, so a chain can never make a level unsolvable — and `badge` is the marker drawn on the obstacle so the chain is visible before you act. **Levels** (`WIZARD_LEVELS`): `id`, `ch`, `name`, `intro`, `obs` (obstacle ids, left to right), optional `boss`, optional `mana` (defaults to 3 + obstacles). A level never names an answer. Run `node validate_wizard.js` (scratchpad) before shipping: it checks solvability at the progress point a child first reaches each level, that no `tip` leaks a solution word, that every category has words, and that the mana budget leaves room for 3★.
- **Rhythm songs** (`RHYTHM_SONGS`): `id`, `name`, `e`, `bpm`, `groove` (a drum pattern from `GROOVES` in js/music.js — also needs an entry in `GROOVE_FEEL` in js/rhythm.js, which is where hard mode gets its note lengths), `chords` (4 scale degrees 0-6, expanded into an 8-bar backing track by `bandDef()`; start on 0 so the loop resolves), 8 `words` of `{ w, zh, e, syl: [...], stress }`. `syl.join('')` must equal `w` and `stress` indexes a real syllable — run `node validate_rhythm.js` (scratchpad). Only pure syllable splits belong here; a wrong split teaches a wrong rhythm.
- **Alchemy recipes** (`ALCHEMY_RECIPES`): `{ ch, a, b, w, zh, e, bonus?, note? }` where a/b are `ALCHEMY_PARTS` ids. **Two hard rules.** (1) `parts[a].text + parts[b].text` must equal `w` exactly — no doubling, no y→i, so what's on screen IS the rule. (2) **Never tell a child a real word doesn't exist**: if two parts on a chapter's shelf spell a real English word it must appear here, as a target or with `bonus: true` (recognised, spoken, rewarded, collected — but not a ??? card and not required to clear the chapter). `node validate_alchemy.js` (scratchpad) enforces both: it fails on any shelf combination that spells a word the site teaches elsewhere (~3,300 words from VOCAB_DATA/wizard/rhythm/pets/fishing, plus every Order Up! menu word AND ingredient id), and `--review` lists the remaining combinations for a human pass. Chapters with `compound: true` get their root+root pairs reviewed too. `ALCHEMY_NEARMISS` gives a real explanation for near-misses that genuinely aren't single words (`noone` → "要寫成兩個字 no one"). `note` covers affixes whose usual gloss doesn't land (dis- in discount means "off", not "not").
- **Order regions** (`ORDER_REGIONS`, 5 × 5 shops): `id`, `name`, `e`, `tip`, `teaches` (`togo`/`temps`/`amt`, or null). **Patterns are inherited cumulatively** — a shop has its own region's pattern plus every earlier region's (`featuresOf` in js/order.js), so a later region structurally cannot forget an earlier one. `tip` is shown once, the first time a child works a shop that introduces the pattern (`save.taught`). `unlockAt` must be **strictly ascending across the whole of `ORDER_SHOPS`** — `shopsOpen()` counts and `save.shops` doubles as an index, so only one shop may open per order served — and shops must stay grouped by region in array order.
- **Order shops** (`ORDER_SHOPS`, base 5 in order.js + 20 in order2.js): `{ id, name, e, unlockAt, region, intro, menu }`; menu items `{ w, zh, e, kind, sizes?, temps?, pl?, art?, def: [], ex: [] }`. `w` is the id, the label AND the TTS script, so multi-word items get a real space (`miso soup`, not `misosoup`) — same for `ORDER_EXTRAS` ids (`soy sauce`, `bbq sauce`). `def` = ingredients the item ships with (what makes "no X" a real action); `ex` = optional add-ons; `pl` only where "two ___" is real English; `temps` = ordered hot/iced. **`art: 'some'` is the only legal `art` value** — a/an is computed by `articleFor()` from the first word actually *spoken* ("an iced tea" but "a hot tea"). `ORDER_EXTRAS[x].amt` marks the uncountable ingredients `extra X`/`less X` reads naturally on; amounts only ever land on `def` ingredients. Hard rules the validator enforces: ≥3 items per shop; every item needs a non-empty `def` **or** `ex` (otherwise the mods budget refunds itself and the shop never drills with/no); `temps` and an `ice` ingredient are mutually exclusive; `art:'some'` excludes `pl`; `w` and emoji unique within a shop; every `def`/`ex` id exists in `ORDER_EXTRAS`. Sentences are generated in js/order.js — modifiers are budgeted per *order*, and `DIFFS.facts` caps how much a child must hold in their head at once (the region's own pattern is exempt: it is mandatory there, not optional load). Run `node validate_order.js` (scratchpad) before shipping — schema plus 25 × 3 × 400 generated-sentence fuzz.
...
- **Rhythm clock**: 英語節奏星 judges against `MusicManager.audioNow()`, not `performance.now()` — the two drift apart and the chart would slowly stop landing on the beat. `MusicManager.startGameTrack(def)` hands the speakers over from the zone playlist (app.js skips `playForZone` for the rhythm zone) and returns `{ startTime, tempo, barDur }`; `stopGameTrack()` gives them back. With music off there is no band and the game falls back to the old clock.
- **Wizard scene rules**: a summon acts on the WHOLE scene, not the blocker in front of the wizard (`sceneEffects`/`applyChains`), and whether a gap is passable is decided geometrically by `walkable()` walking the surface profile — never by a lookup table. Tags set the summoned box's size and buoyancy (`bodyOf`/`orientBody`), so `float`/`heavy`/`long`/`climb` are physical properties. Every timer the module schedules goes through `later()`, which drops it if the attempt has been restarted.
...
| Word wizard (per summon, spelled correctly) | 10 XP + 1 gem | 14 XP + 1 gem | 18 XP + 2 gems |
