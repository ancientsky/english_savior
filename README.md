...
| 🐾 單字寵物島 | 答題捕捉 90 種單字寵物、進化學新單字、15 座道館對戰 | 單字 |
...
| 🪄 單字魔法師 | 塗鴉冒險家式召喚解謎：拼出單字召喚該物體、自己選落點，真物理＋幾何判定決定過不過，30 關每關多解 | 單字拼寫/字彙廣度 |
| 🎵 英語節奏星 | 太鼓達人式音節節奏，重音是長音、輕音是短音，20 首各有伴奏，160 字 | 發音/音節重音 |
| 🔤 單字鍊金術 | 字首＋字根丟進坩堝合成新單字，16 章 282 個字＋限時挑戰 | 構詞規則 |
| 🛎️ 英語打工大亂鬥 | 聽客人整句英文點餐／點服務，親手加料、拿掉、挑尺寸顏色、選時間做出來；🍜 餐飲 25 家店 ＋ 🏪 生活服務 20 個櫃台，9 個區域各教一種新句型 | 聽力理解 |
...
- **成就系統**：71 個成就（含隱藏成就）共 1,410 點，點數門檻解鎖專屬獎勵
...
- **魔法師單字/關卡**：`js/data/wizard.js` → `WIZARD_WORDS`（12 種能力標籤 ＋ 7 種分類）與 `WIZARD_LEVELS`（關卡只列障礙，解法由障礙的標籤決定；障礙的 `tip` 只描述情境、`hint` 才是解法）
- **節奏星曲目**：`js/data/rhythm.js` → `RHYTHM_SONGS`（音節切分＋重音位置＋`groove` 鼓組＋`chords` 四個和弦）
- **鍊金配方**：`js/data/alchemy.js` → `ALCHEMY_PARTS` 與 `ALCHEMY_RECIPES`（只收純串接的組合；零件湊得出來的真英文字一律要收，`bonus: true` 表示不是該章目標）
...
- 成就：90 個（10/20/30/40 點四檔，共 1,820 點；點數門檻贈專屬稱號/造型/主題）
